...
| 컴포넌트 | 역할 |
| :--- | :--- |
| ECS Fargate (1–4 task) | FastAPI 애플리케이션 + WebSocket 호스팅 |
| Application Load Balancer | `/api/v1/*`, `/ws/realtime`, `/admin/*` 라우팅 |
| RDS Postgres 15 + TimescaleDB | 관계형 + 하이퍼테이블(이벤트, 감사 로그, 생체신호 메타데이터) |
| S3 (Seoul, KMS 암호화) | 옵트인 암호화 블롭 보관 |
| EventBridge Scheduler + ECS RunTask | 주기적 정리 잡(`purge_accounts` 매일 03:00 UTC, `purge_biosignals` 6시간) |
| SQS DLQ + CloudWatch Alarm | 스케줄러 실패 격리·알람 |
| ECR | 컨테이너 이미지 레지스트리 |
| Secrets Manager | DB 비밀번호, Supabase JWKS, FCM 키 |

전체 정의는 [`backend/infra/`](backend/infra/) — `ecs.tf`, `alb.tf`, `rds.tf`, `s3.tf`, `scheduler.tf`, `secrets.tf`, `networking.tf`, `ecr.tf`.
